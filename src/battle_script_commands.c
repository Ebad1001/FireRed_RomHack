...
static void Cmd_critcalc(void)
{
    ...
    critChance  = 2 * ((gBattleMons[gBattlerAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0)
                ...
                + (gBattleMoves[gCurrentMove].effect == EFFECT_AQUA_CUTTER)
...
{
    // Type Effectiveness is Calculated here
            ...
            if (statsToCheck & 1)
            {
                if (gBattlescriptCurrInstr[3] & STAT_CHANGE_CANT_PREVENT)
                {
                    if (gBattleMons[gActiveBattler].statStages[currStat] > MIN_STAT_STAGE)
                    {
                        ...
                        && !(gBattleMons[gActiveBattler].ability == ABILITY_HYPER_CUTTER && currStat == STAT_SPATK)
    ...
    if (statValue <= -1) // Stat decrease.
    {
                 ...
                 && !certain && statId == STAT_ACC)
        {
            ...
            return STAT_CHANGE_DIDNT_WORK;
              ...
              || (gBattleMons[gActiveBattler].ability == ABILITY_HYPER_CUTTER && !certain && statId == STAT_SPATK))
        {
            if (flags == STAT_CHANGE_ALLOW_PTR)
            {
                BattleScriptPush(BS_ptr);
