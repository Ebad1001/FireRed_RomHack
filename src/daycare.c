...
static void AlterEggSpeciesWithIncenseItem(u16 *species, struct DayCare *daycare)
{
    ...
    if (*species == SPECIES_FORKNIGHT || *species == SPECIES_SPARKITE)
    {
        motherItem = GetBoxMonData(&daycare->mons[0].mon, MON_DATA_HELD_ITEM);
        fatherItem = GetBoxMonData(&daycare->mons[1].mon, MON_DATA_HELD_ITEM);
        if (*species == SPECIES_FORKNIGHT && motherItem != ITEM_LAX_INCENSE && fatherItem != ITEM_LAX_INCENSE)
        {
            *species = SPECIES_WOBBUFFET;
        }

        if (*species == SPECIES_SPARKITE && motherItem != ITEM_SEA_INCENSE && fatherItem != ITEM_SEA_INCENSE)
        {
...
static u16 DetermineEggSpeciesAndParentSlots(struct DayCare *daycare, u8 *parentSlots)
{
    ...
    if (eggSpecies == SPECIES_WILDMUTT && daycare->offspringPersonality & EGG_GENDER_MALE)
    {
        eggSpecies = SPECIES_POCHITA;
    }
    if (eggSpecies == SPECIES_MILTANK && daycare->offspringPersonality & EGG_GENDER_MALE)
    {
